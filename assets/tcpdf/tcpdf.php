...
class TCPDF {
	...
	public $ft_dt_complete = '';

	public $ft_id_co = '';

	public function set_ft_id_co($value='')
	{
		$this->ft_id_co = $value;
	}

	public function get_ft_id_co()
	{
		return $this->ft_id_co;
	}

	public function set_ft_dt_create($value='')
	{
		$this->ft_dt_create = $value;
	}

	public function get_ft_dt_create()
	{
		return $this->ft_dt_create;
	}

	public function set_ft_dt_complete($value='')
	{
		$this->ft_dt_complete = $value;
	}

	public function get_ft_dt_complete()
	{
		return $this->ft_dt_complete;
	}
	 ...
	 * @protected
	 */
	protected $header_logo_width = LOGO_HEADER_WIDTH;
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($newimage) {
			...
			$info = false;
			if ((method_exists('TCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick2'))))) {
			...
			if (($info === false) AND extension_loaded('imagick2')) {
				try {
					// ImageMagick library
					$img = new Imagick();
					if ($type == 'SVG') {
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if (($parsed === false) AND extension_loaded('imagick2')) {
			try {
				// ImageMagick library
				$img = new Imagick();
				$img->readImage($file);
	...
	public function addHTMLTOC($page='', $toc_name='TOC', $templates=array(), $correct_align=true, $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			if (TCPDF_STATIC::empty_string($page)) {
				...
				// placemark to be replaced with the correct number
				$c_pagenum = $outline['p'] + 1;
				if((int)$c_pagenum < 10) {
					$c_pagenum = '0'.$c_pagenum;
				}
				$pagenum = '{#'.($c_pagenum).'}';
			...
			$outline_text = explode(':::', $outline['t']);

			$row = str_replace('#TOC_DESCRIPTION#', $outline_text[0], $row);
			if(isset($outline_text[1])) {
				$row = str_replace('#TOC_DETAIL#', $outline_text[1], $row);
			}
...
class MYPDF extends TCPDF {
    public function Footer($ID_COM = '') 
    {
        $image_file = (dirname(__FILE__).'/img_report/footer_line.jpg');
        $this->Image($image_file, 15, 270, 185, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
		///
        $this->SetY(-24);
        $this->SetFont('helvetica', 'N', 12);
        $this->setCellPaddings(5,4,0,0);
        $this->Cell(0, 5, "Company ID: ".$this->ft_id_co, 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->setCellPaddings(0,8,0,0);
        $this->Cell(0, 5, "Date of Report: ".$this->ft_dt_create, 0, false, 'R', 0, '', 0, false, 'T', 'M');
        
        $this->SetY(-14);
        $this->SetFont('helvetica', 'N', 10);
        $this->setCellPaddings(5,0,0,0);
        $this->Cell(0, 5, "© ".date('Y').", Hay Group Pte Ltd. All Rights Reserved.", 0, false, 'L', 0, '', 0, false, 'T', 'M');
        $this->setCellPaddings(5,0,-10,0);
        $this->Cell(18, 5, $this->getAliasNumPage(), 0, false, 'R', 0, '', 0, false, 'T', 'M');
