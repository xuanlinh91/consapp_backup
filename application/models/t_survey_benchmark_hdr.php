...
/**
 *T_User
 *Description: model manage t_survey_benchmark_hdr
 * @author quanhm
 */
Class T_survey_benchmark_hdr extends MY_Model
{
    function __construct()
    {
        parent::__construct();
        $this->table_name = 't_survey_benchmark_hdr';
    }
    ...
    function get_data_by_id($select = '*', $id = null)
    {
        ...
        if (!is_null($id) && is_numeric($id)) {
            $this->db->where("id_maturity = $id LIMIT 1");
        } else {
            return null;
        }
        //select data
